...
NOTE: *Path 1 (Hyper-V path) vs Path 2 (Azure Local VM path):* On Path 1, Arc registration happens automatically via the platform's VM Config Agent after `03-Convert-Gen1toGen2.ps1` completes. The steps below address that auto-registration failing. On *Path 2*, Arc registration is intentional and explicit — run `05-Reconnect-AzureLocalVM.ps1` rather than waiting for auto-registration. If script 05 fails, re-run it directly.
...
NOTE: This fallback applies to *Path 1* only. If you are on *Path 2*, re-run `05-Reconnect-AzureLocalVM.ps1` instead.

*Step 5:* If the VM still doesn't appear after 15 minutes with the resource bridge healthy, delete the stale Arc VM resource from Azure (if one exists) and re-run the `az stack-hci-vm create` fallback command (*Path 1*) or re-run `05-Reconnect-AzureLocalVM.ps1` (*Path 2*).
