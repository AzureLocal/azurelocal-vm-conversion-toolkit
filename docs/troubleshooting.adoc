...
*Symptom:* After Gen 2 conversion and VM boot, the VM doesn't appear in Azure portal as an Arc-enabled server within 10 minutes.

*Step 1:* Restart the VM Config Agent on the Azure Local host node:
...
*Step 3:* Use the manual fallback command output by `03-Convert-Gen1toGen2.ps1` at the end of its run. It prints a full `az stack-hci-vm create` command you can re-run.

*Step 4:* Verify the Arc agent inside the guest is running:
