...
*Step 4:* Use the manual fallback command output by `03-Convert-Gen1toGen2.ps1` at the end of its run. It prints a full `az stack-hci-vm create` command you can re-run to re-project the VM resource into Azure.

*Step 5:* If the VM still doesn't appear after 15 minutes with the resource bridge healthy, delete the stale Arc VM resource from Azure (if one exists) and re-run the `az stack-hci-vm create` fallback command.
