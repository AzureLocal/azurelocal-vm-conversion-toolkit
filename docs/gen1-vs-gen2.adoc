...
| Same as Path 1 (reconnect step adds minimal time, no additional downtime)
...
NOTE: Both paths are workload-preserving. Path 2 adds one extra step (`reconnect-to-azure`) after the Gen 2 conversion — it does not require Sysprep or any identity-destructive operation.
