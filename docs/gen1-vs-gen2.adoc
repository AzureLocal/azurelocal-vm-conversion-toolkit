...
|===
| Consideration | Path 1 — Hyper-V Cluster | Path 2 — Azure Local VM
...
| ✗ No — Sysprep destroys machine identity

| *Machine SID / domain join*
| ✓ Preserved
| ✗ Lost — must re-join domain and reinstall apps

| *Downtime*
| 5–15 minutes + VHDX backup time
| Full maintenance window (hours)

| *Rollback*
| VHDX backup restore
| Restore pre-sysprep VHDX backup
...
| Automatic via Arc resource bridge (platform-managed)

| *Best for*
| VMs that must stay online with minimal disruption; applications with machine-bound state
| New standardized VM deployments; workloads that can be cleanly re-deployed from a baseline image
|===

=== Use Path 1 (Hyper-V) if:

* You need minimal downtime
* The application has machine-bound licensing or identity requirements
* You need to preserve domain join, local user accounts, and installed application state
* Portal-based VM management is not a requirement

=== Use Path 2 (Azure Local VM) if:
...
* The workload can be cleanly redeployed from a generalized image
* You have an existing patching/deployment process that starts from a sysprepped base image

NOTE: If you are unsure which path to take, start with Path 1. It is lower risk and reversible. You can always migrate from a Gen 2 Hyper-V VM to a proper Azure Local VM later using the same sysprep workflow.
...
* xref:runbook-azurelocal.adoc[Runbook: Azure Local VM Path] — portal-managed VM, requires Sysprep, scripts 01–02 + 05–06
