...
Run from the cluster node after the guest VM is shut down. This backs up the VHDXs, removes the Gen 1 VM, recreates it as Gen 2, re-adds it to the failover cluster, and re-registers it with Azure Arc.
