...
There is no in-place upgrade path from Hyper-V Gen 1 to Gen 2. This toolkit automates the manual process of converting VMs by:

1. Inventorying Gen 1 VMs and exporting their full configurations
2. Converting boot disks from MBR to GPT inside the guest OS
3. Recreating each VM as Gen 2 with identical settings (CPU, memory, NICs, VLANs, disks)
4. Re-adding VMs to the failover cluster
5. Re-registering VMs with Azure Arc so they remain managed through the Azure portal
...
| [`scripts/cluster/03-Convert-Gen1toGen2.ps1`](scripts/cluster/03-Convert-Gen1toGen2.ps1) | Azure Local cluster node | Removes Gen 1 VM, creates Gen 2 VM with same config, re-clusters, re-registers with Arc |
...
| Doc | Description |
|-----|-------------|
| [docs/gen1-vs-gen2.adoc](docs/gen1-vs-gen2.adoc) | **Start here** — Gen 1 vs Gen 2 decision guide, feature comparison, and conversion checklist |
| [docs/runbook.adoc](docs/runbook.adoc) | Step-by-step runbook — workflow, commands, working directory structure |
