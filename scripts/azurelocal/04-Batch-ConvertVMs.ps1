...
<#
.SYNOPSIS
    Batch orchestrator for converting multiple Gen 1 VMs to Gen 2 on Azure Local.
...
.DESCRIPTION
    ...
    before this batch script can process them. This script validates that
    the guest conversion was completed.

.PARAMETER WorkingDirectory
    Path to the conversion working directory.
...
.PARAMETER VMNames
    Optional array of specific VM names to convert. If omitted, shows all Gen 1 VMs.

.PARAMETER MaxParallel
    Number of VMs to process in parallel. Default 1 (sequential) for safety.
...
Write-Log "═══════════════════════════════════════════════════════════════"
Write-Log "  Azure Local Batch Gen1 → Gen2 Conversion"
Write-Log "═══════════════════════════════════════════════════════════════"
...
Write-Host "║  - Re-register with Azure Arc                                ║" -ForegroundColor Yellow
Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Yellow
Write-Host ""
...
foreach ($vmEntry in $readyVMs) {
    ...
    try {
        ...
        $params = @{
            ...
            SkipArcRegistration = $SkipArcRegistration.IsPresent
            BackupVHDX          = $true
            Confirm             = $false
        }
